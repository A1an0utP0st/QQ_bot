...
headers = {
    ...
    'Authorization': 'Bearer eyJhbGciOiJFUzUxMiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjI2NDA5IiwiZW1haWwiOiIyNzkyMDY3NDY3QHFxLmNvbSIsInB1cnBvc2UiOiJ3ZWIiLCJpYXQiOjE3MDc3MDc2MjUsImV4cCI6MTcwODkxNzIyNX0.ACvMQ41h-E36wBCmCNH9pHQ_yUjP59wVNwbvngEIFRy_hPmd0pfr9ODbT_q46VGyd7meTNcIfwxIcMKTrnoSUcfYACZ3n13-3UWBS9sO213WZT-ep7DU0If57epoIenxpZw0ksRj8l81CWj9NH4Pu-MBPtOfa__1QBv0ebIjxr0TqZhd',  # 改左边
...
messageList = [
    rc("system",
       "You are 张天祺, a chat robot trained by A1an,you can execute many instructions start with '/', such as '/e 1+1','/匹配 A1an',your official group is 301459606."),
]
...
def makedata(thisinput: str = "", thisuser: str = "user", lastuser: str = "user", lastinput: str = "",
    ...
    return {
        "session_id": "591bb192-b052-433e-be42-5b39d7bc09fa",  # 改左边
...
async def _(state: T_State, arg: Message = CommandArg()):
    ...
    messageList = [
        rc("system",
           "You are 七七, a chat robot trained by 鼠鼠.You can execute many instructions start with '/', such as '/e','/匹配',your official group is 817982142."),
    ]
...
@abstract.got("i", prompt="请输入新身份的名字（如七七）：")
async def _(bot: Bot, event: Event, i: str = ArgStr("i")):
    ...
    messageList = [
        rc("system",
           f"You are {i},your master is 鼠鼠.You can execute many instructions start with '/',your official group is 817982142."),
    ]
